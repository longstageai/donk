...
class EmbeddingConfigPage extends StatefulWidget {
  const EmbeddingConfigPage({super.key});

  @override
  State<EmbeddingConfigPage> createState() => _EmbeddingConfigPageState();
}

class _EmbeddingConfigPageState extends State<EmbeddingConfigPage> {
  /// 是否正在加载
  ...
  Future<void> _loadConfig() async {
    ...
    try {
      ...
      final provider = data['embedding_provider'] ?? 'openai';
      final config = _providerConfigs[provider] ?? _providerConfigs['openai']!;
      _providerController.text = provider;
      _modelController.text =
          data['embedding_model'] ?? config['defaultModel']!;
      _apiKeyController.text = data['embedding_api_key'] ?? '';
      _baseUrlController.text =
          data['embedding_base_url'] ?? config['defaultBaseUrl']!;
          ...
          Container(
            ...
            child: const Row(
              children: [
                ...
                Expanded(
                  child: Text(
                    '注意：Embedding 配置只能设置一次，不同的 Embedding 模型维度不兼容，切换后需要重新处理所有文档',
