...
Donk 是一个本地优先的 AI Agent 桌面应用，由 Go 后端服务和 Flutter Windows 客户端组成。它把大模型对话、工具调用、Skill 扩展、本地知识库、长期记忆、任务调度、实时通知和桌面交互整合到一个可运行、可扩展的个人工作流平台中。

Donk 的目标不是再做一个聊天窗口，而是提供一个完整的本地 Agent 运行环境：用户通过桌面端和 Agent 对话，Agent 可以读取知识库、调用工具、执行 Skill 脚本、保存记忆、创建定时任务，并把后台事件实时推送回客户端。
